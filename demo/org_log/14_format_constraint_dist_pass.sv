...
function automatic int max(const ref int int_q[$]);
    `DV_CHECK_GT_FATAL(
        int_q.size(),
        0,
        "max function cannot accept an empty queue of integers!",
        $sformatf("%m")
    )
    // Assign the first value from the queue in case of negative integers.
    max        = int_q[0];
    foreach (int_q[i])
        max        = max2(max, int_q[i]);
    return max;
endfunction
...
function automatic bit [bus_params_pkg::BUS_DW-1:0] get_masked_data(bit [bus_params_pkg::BUS_DW-1:0] data, bit [bus_params_pkg::BUS_DBW-1:0] mask, uvm_reg csr = null);
    bit                 [bus_params_pkg::BUS_DW-1:0] csr_data                            ;
    csr_data   = (csr!=null)?csr.get_mirrored_value():'0;
    get_masked_data = data;
    foreach (mask[i]) begin
        if (~mask[i])
            get_masked_data[i*8+:8] = csr_data[i*8+:8];
    end
endfunction
...
function automatic bit sw_symbol_get_addr_size(input string elf_file, input string symbol, input bit does_not_exist_ok, output longint unsigned addr, output longint unsigned size);

    string escaped_symbol = "";
    string symbol_for_filename = "";

    if (elf_file=="")
        `uvm_fatal($sformatf("%m"), "'elf_file' argument cannot be an empty string")
    if (symbol=="")
        `uvm_fatal($sformatf("%m"), "'symbol' argument cannot be an empty string")
    ...
    begin
        ...
        // `\s%0s$` ensures we are looking for an exact match, with no pre- or postfixes."/usr/bin/readelf -s --wide %0s | grep \"\\s%0s$\" | awk \'{print $2\" \"$3}\' > %0s",
                               elf_file,
                               escaped_symbol,
                               out_file);
        ...
        `DV_CHECK_EQ_FATAL(
            ret,
            0,
            $sformatf("Command \"%0s\" failed with exit code %0d", cmd, ret),
            $sformatf("%m")
        )
        ...
        `DV_CHECK_EQ_FATAL(
            ret,
            2,
            $sformatf("Failed to extract {addr size} from line \"%0s\"", line),
            $sformatf("%m")
        )
...
$sformatf("%m")) $fclose(out_file_d);

        ret        = $system($sformatf("rm -rf %0s", out_file));
        `DV_CHECK_EQ_FATAL(
            ret,
            0,
            $sformatf("Failed to delete \"%0s\"", out_file),
            $sformatf("%m")
        )
        return 1;
    end
...
function automatic void read_vmem(input string vmem_file, output logic [bus_params_pkg::BUS_DW-1:0] vmem_data[$]);
    int                 fd                                  ;
    uvm_reg_addr_t      last_addr                           ;
    string text, msg = "\n", lines[$];

    fd         = $fopen(vmem_file, "r");
    `DV_CHECK_FATAL(fd, $sformatf("Failed to open \"%0s\"", vmem_file), $sformatf("%m")) while (!$feof(fd)) begin
        string line;
        void'($fgets(line, fd));
    ...
    text       = str_utils_pkg::str_remove_sections(
                                    .s(text),
                                    .start_delim("/*"),
                                    .end_delim("*/")
                                );
...
lines[i], vmem_file), $sformatf("%m")) if (!str_utils_pkg::str_starts_with(tokens[0], "@")) begin
            `uvm_fatal($sformatf("%m"), $sformatf({"The first word \"%s\" on line \"%s\" in the VMEM", " file %s does not appear to be a valid address"}, tokens[0], lines[i], vmem_file))
        end
...
bus_params_pkg::BUS_AW / 8, tokens[0]), $sformatf("%m")) addr = tokens[0].atohex();
        tokens     = tokens[1:$];
        if (i>0) begin
...
bus_params_pkg::BUS_DW / 8, tokens[i]), $sformatf("%m")) data = tokens[i].atohex();
            msg        = {msg, $sformatf(
                                   "[0x%0h] = 0x%0h\n",
                                   addr+i,
                                   data
