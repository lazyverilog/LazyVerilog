...
function automatic void endian_swap_byte_arr(
    ref bit [7:0] arr[]
);
    arr        = {<<byte {arr}};
    arr        = {<<32{arr}};
...
function automatic string get_parent_hier(
    string hier,
    int n_levels_up = 1
);
    int                 idx                                 ;
...
task automatic poll_for_stop(
    uint interval_ns = 10_000,
    string filename = "dv.stop"
);
    fork
        while (1) begin
            #(interval_ns*1ns);
...
function automatic bit sw_symbol_get_addr_size(
...
function automatic void read_vmem(
    input string vmem_file,
