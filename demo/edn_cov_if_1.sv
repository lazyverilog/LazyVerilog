...
covergroup edn_cfg_cg with function sample(uint num_endpoints, uint num_boot_reqs, mubi4_t boot_req_mode, mubi4_t auto_req_mode);
...
endgroup: edn_cfg_cg
    ...
    covergroup edn_cs_cmds_cg with function sample(bit [3:0] clen, bit [3:0] flags, bit [18:0] glen, csrng_pkg::acmd_e acmd, edn_env_pkg::cmd_type_e cmd_src, edn_env_pkg::hw_req_mode_e mode);
        ...
        cr_generate_intended: cross cp_acmd, cp_clen, cp_glen, cp_mode, cp_cmd_src {
                ...
                ignore_bins gen_auto_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::AutoReqMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::AutoGen};
                // Generate commands in boot mode that aren't from the bootgen register aren't intended
                ignore_bins gen_boot_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::BootReqMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::BootGen};
                // Generate commands in boot mode that have a clen > 0 aren't intended
                ignore_bins gen_boot_seq_wrong_clen = binsof (cp_mode) intersect {edn_env_pkg::BootReqMode}&&binsof (cp_cmd_src) intersect {edn_env_pkg::BootGen}&& !binsof (cp_clen) intersect {0};
                // Generate commands in boot mode that have a glen of multiple words aren't intended
                ignore_bins gen_boot_seq_wrong_glen = binsof (cp_mode) intersect {edn_env_pkg::BootReqMode}&&binsof (cp_cmd_src) intersect {edn_env_pkg::BootGen}&& !binsof (cp_glen) intersect {[2:$]};
                // Generate commands in sw mode that aren't from the sw register aren't intended
                ignore_bins gen_sw_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::SwMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::Sw};
            }
        ...
        cr_instantiate_intended: cross cp_acmd, cp_clen, cp_flags, cp_mode, cp_cmd_src {
                ...
                ignore_bins ins_auto_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::AutoReqMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::Sw};
                // Instantiate commands in boot mode that aren't from the BootIns register aren't intended
                ignore_bins ins_boot_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::BootReqMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::BootIns};
                // Instantiate commands in boot mode that have a clen > 0 aren't intended
                ignore_bins ins_boot_seq_wrong_clen = binsof (cp_mode) intersect {edn_env_pkg::BootReqMode}&&binsof (cp_cmd_src) intersect {edn_env_pkg::BootIns}&& !binsof (cp_clen) intersect {0};
                // Instantiate commands in boot mode that have a flag0 = MuBi4True aren't intended
                ignore_bins ins_boot_seq_wrong_flag0 = binsof (cp_mode) intersect {edn_env_pkg::BootReqMode}&&binsof (cp_cmd_src) intersect {edn_env_pkg::BootIns}&& !binsof (cp_flags) intersect {MuBi4False};
                // Instantiate commands in sw mode that aren't from the sw register aren't intended
                ignore_bins ins_sw_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::SwMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::Sw};
            }
        ...
        cr_reseed_intended: cross cp_acmd, cp_clen, cp_flags, cp_mode, cp_cmd_src {
                ...
                ignore_bins res_auto_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::AutoReqMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::AutoRes};
                ...
                ignore_bins res_sw_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::SwMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::Sw};
            }
        ...
        cr_update_intended: cross cp_acmd, cp_clen, cp_mode, cp_cmd_src {
                ...
                ignore_bins upd_sw_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::SwMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::Sw};
            }
        ...
        cr_uninstantiate_intended: cross cp_acmd, cp_mode, cp_cmd_src {
                ...
                ignore_bins uni_sw_wrong_src = binsof (cp_mode) intersect {edn_env_pkg::SwMode}&& !binsof (cp_cmd_src) intersect {edn_env_pkg::Sw};
            }
        ...
        endgroup: edn_error_cg
            ...
            endgroup: edn_alert_cg
                ...
                endgroup: edn_cs_cmd_response_cg
                    covergroup edn_sw_cmd_sts_cg with function sample(bit cmd_rdy, bit cmd_reg_rdy, csrng_pkg::csrng_cmd_sts_e cmd_sts, bit cmd_ack);
                    ...
                    endgroup: edn_sw_cmd_sts_cg
                        covergroup edn_hw_cmd_sts_cg with function sample(bit boot_mode, bit auto_mode, csrng_pkg::csrng_cmd_sts_e cmd_sts, bit cmd_ack, csrng_pkg::acmd_e acmd);
                            ...
                            cr_acmd_boot_mode: cross cp_acmd, cp_boot_mode {
                                    ...
                                    ignore_bins not_valid_boot_commands = !binsof (cp_acmd) intersect {csrng_pkg::INS, csrng_pkg::GEN, csrng_pkg::UNI};
                                }
                            ...
                            cr_acmd_auto_mode: cross cp_acmd, cp_auto_mode {
                                    ...
                                    ignore_bins not_valid_boot_commands = !binsof (cp_acmd) intersect {csrng_pkg::RES, csrng_pkg::GEN};
                                }
                        endgroup: edn_hw_cmd_sts_cg
                            ...
                            endgroup: edn_endpoint_err_req_cg
                                ...
                                function automatic void cg_cfg_sample(
                                    edn_env_cfg cfg
                                );
                                    edn_cfg_cg_inst.sample(
                                                        cfg.num_endpoints,
                                                        cfg.num_boot_reqs,
                                                        cfg.boot_req_mode,
                                                        cfg.auto_req_mode
                                ...
                                function automatic void cg_cs_cmds_sample(
                                ...
                                function automatic void cg_error_sample(
                                    bit [31:0] err_code
                                );
                                ...
                                function automatic void cg_alert_sample(
                                    bit [31:0] recov_alert_sts
                                );
                                ...
                                function automatic void cg_cs_cmd_response_sample(
                                    csrng_pkg::csrng_cmd_sts_e csrng_rsp_sts,
                                    bit csrng_rsp_ack
                                );
                                ...
                                function automatic void cg_edn_sw_cmd_sts_sample(
                                    bit cmd_rdy,
                                    bit cmd_reg_rdy,
                                    csrng_pkg::csrng_cmd_sts_e cmd_sts,
                                    bit cmd_ack
                                );
                                    edn_sw_cmd_sts_cg_inst.sample(
                                                               cmd_rdy,
                                                               cmd_reg_rdy,
                                                               cmd_sts,
                                                               cmd_ack
                                ...
                                function automatic void cg_edn_hw_cmd_sts_sample(
                                    bit boot_mode,
                                    bit auto_mode,
                                    csrng_pkg::csrng_cmd_sts_e cmd_sts,
                                    bit cmd_ack,
                                ...
                                function automatic void cg_edn_endpoint_err_req_sample(
                                    uint endpoint
                                );
