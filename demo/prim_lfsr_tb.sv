...
for (genvar k=MinLfsrDw ; k<=MaxLfsrDw ; k++) begin: gen_duts
    ...
    // DUTs
    //////////////////////////////////////////////////////
    prim_lfsr #(.LfsrType(LfsrType), .LfsrDw(k), .EntropyDw(k), .StateOutDw(k), .DefaultSeed(k'(SEED)),
    // The case where this is disabled is already tested with FPV.
    // Hence we cover the enabled case with custom permutations
    // in this testbench.
    .StatePermEn(1'b1), .StatePerm(StatePerm[MaxLfsrDw-1:0]),
    // Enable internal max length check.
    .MaxLenSVA(1'b1)) i_prim_lfsr(.clk_i(clk),
                                  .rst_ni(rst_n),
                                  .seed_en_i(seed_en[k]),
                                  .seed_i(seed[k][k-1:0]),
                                  .lfsr_en_i(lfsr_en[k]),
    ...
    initial begin: p_stimuli
        ...
        $display("Starting LFSR maxlen test for width %0d: running %0d cycles",
                 k,
                 2**k-1);
        for (longint unsigned i=0 ; i<=lfsr_periods[MaxLfsrDw]&&lfsr_en[k] ; i++) begin
            ...
            if (state_out[k]==actual_default_seed&&lfsr_en[k]) begin
                ...
                if (i==lfsr_periods[k]) begin
                    ...
                    $error("LFSR maxlen test for width %0d failed at period %0d!",
                           k,
                           i);
                    err[k]=1'b1;
                end
        ...
        if (gen_duts[k].i_prim_lfsr.lfsr_q==actual_default_seed) begin
            ...
            $error("LFSR with width %0d does not reset to DefaultSeedLocal when lockup is detected",
                   k);
            err[k]=1'b1;
        end
