...
interface entropy_src_cov_if import entropy_src_pkg::*;
import prim_mubi_pkg::*;
(input logic clk_i, input logic rst_ni, mubi8_t otp_en_entropy_src_fw_read_i, mubi8_t otp_en_entropy_src_fw_over_i);
...
endgroup: err_test_cg
    ...
    endgroup: mubi_err_cg
        ...
        endgroup: sm_err_cg
            covergroup fifo_err_cg with function sample(which_fifo_err_e which_fifo_err, which_fifo_e which_fifo);
                ...
                cr_fifo_err: cross cp_which_fifo, cp_which_err {
                        ignore_bins no_write_err = binsof (cp_which_fifo) intersect {sfifo_esfinal, sfifo_esrng}&&binsof (cp_which_err) intersect {write};
                    }
            endgroup: fifo_err_cg
                covergroup cntr_err_cg with function sample(cntr_e which_cntr, int which_line, int which_bucket);
                ...
                endgroup: cntr_err_cg
                    ...
                    covergroup seed_output_csr_cg with function sample(mubi4_t fips_enable, mubi4_t fips_flag, mubi4_t rng_fips, mubi4_t threshold_scope, mubi4_t rng_bit_enable, bit [1:0] rng_bit_sel, mubi4_t es_route, mubi4_t es_type, mubi4_t entropy_data_reg_enable, bit [7:0] otp_en_es_fw_read, mubi4_t fw_ov_mode, bit [7:0] otp_en_es_fw_over, mubi4_t entropy_insert, bit full_seed);
                        ...
                        cp_rng_bit: coverpoint {
                                rng_bit_enable==MuBi4True, rng_bit_sel
                            }
                        iff (full_seed) {bins enabled[] = {[3'b100:3'b111]};
                        ...
                        cr_config: cross cp_fips_enable, cp_fips_flag, cp_rng_fips, cp_threshold_scope, cp_rng_bit, cp_es_type {
                                // The threshold scope needs to be set to false if we are in the single lane mode.
                                ignore_bins thresh_scope_true_rng_bit_true = binsof (cp_threshold_scope) intersect {MuBi4True}&&binsof (cp_rng_bit) intersect {[3'b100:3'b111]};
                            }
                            // Finer crosses
                    ...
                    endgroup: seed_output_csr_cg
                        ...
                        // for all configurations
                        covergroup csrng_hw_cg with function sample(bit [3:0] fips_enable, bit [3:0] fips_flag, bit [3:0] rng_fips, bit [3:0] threshold_scope, bit [3:0] rng_bit_enable, bit [1:0] rng_bit_sel, bit [3:0] es_route, bit [3:0] es_type, bit [3:0] entropy_data_reg_enable, bit [7:0] otp_en_es_fw_read, bit [3:0] fw_ov_mode, bit [7:0] otp_en_es_fw_over, bit [3:0] entropy_insert);
                            ...
                            cr_config: cross cp_fips_enable, cp_threshold_scope, cp_rng_bit_enable, cp_es_type, cp_entropy_data_reg_enable, cp_otp_en_es_fw_read, cp_otp_en_es_fw_over {
                                    // Ignore the invalid mubi values.
                                    ignore_bins otp_en_es_fw_read_inval = !binsof (cp_otp_en_es_fw_read) intersect {MuBi8True, MuBi8False};
                                    ignore_bins otp_en_es_fw_over_inval = !binsof (cp_otp_en_es_fw_over) intersect {MuBi8True, MuBi8False};
                                    // The threshold scope needs to be set to false if we are in the single lane mode.
                                    ignore_bins thresh_scope_true_rng_bit_true = binsof (cp_threshold_scope) intersect {MuBi4True}&&binsof (cp_rng_bit_enable) intersect {MuBi4True};
                                }
                                // Smaller crosses
                            cr_fips_scope_type: cross cp_fips_enable, cp_threshold_scope, cp_es_type;
                            cr_fips_scope_data_enable: cross cp_fips_enable, cp_threshold_scope, cp_entropy_data_reg_enable;
                            cr_fips_scope_otp: cross cp_fips_enable, cp_threshold_scope, cp_otp_en_es_fw_read, cp_otp_en_es_fw_over;
                            ...
                            cr_scope_bit_otp: cross cp_threshold_scope, cp_rng_bit, cp_otp_en_es_fw_read, cp_otp_en_es_fw_over {
                                    // The threshold scope needs to be set to false if we are in the single lane mode.
                                    ignore_bins thresh_scope_true_rng_bit_true = binsof (cp_threshold_scope) intersect {MuBi4True}&&binsof (cp_rng_bit) intersect {[3'b100:3'b111]};
                                }
                        ...
                        endgroup: csrng_hw_cg
                            ...
                            // for all configurations
                            covergroup observe_fifo_event_cg with function sample(mubi4_t fips_enable, mubi4_t fips_flag, mubi4_t rng_fips, mubi4_t threshold_scope, mubi4_t rng_bit_enable, bit [1:0] rng_bit_sel, mubi4_t es_route, mubi4_t es_type, mubi4_t entropy_data_reg_enable, bit [7:0] otp_en_es_fw_read, mubi4_t fw_ov_mode, bit [7:0] otp_en_es_fw_over, mubi4_t entropy_insert);
                                ...
                                cr_config: cross cp_fips_enable, cp_threshold_scope, cp_rng_bit, cp_es_route, cp_es_type, cp_entropy_data_reg_enable, cp_otp_en_es_fw_read, cp_otp_en_es_fw_over {
                                        // The threshold scope needs to be set to false if we are in the single lane mode.
                                        ignore_bins thresh_scope_true_rng_bit_true = binsof (cp_threshold_scope) intersect {MuBi4True}&&binsof (cp_rng_bit) intersect {[3'b100:3'b111]};
                                    }
                                ...
                                cr_rng_insert_otp: cross cp_rng_bit, cp_entropy_insert, cp_otp_en_es_fw_read, cp_otp_en_es_fw_over;
                            endgroup: observe_fifo_event_cg
                                covergroup sw_update_cg with function sample(uvm_reg_addr_t offset, bit sw_regupd, bit module_enable);
                                    option.name = "sw_update_cg";
                                    option.per_instance = 1;
                                    cp_lock_state: coverpoint {
                                            sw_regupd, module_enable
                                        }
                                    {bins locked_states[] = {2'b00, 2'b01, 2'b11};
                                    }
                                    cp_offset: coverpoint offset {
                                            bins lockable_offsets[] = {entropy_src_reg_pkg::ENTROPY_SRC_CONF_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_ENTROPY_CONTROL_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_HEALTH_TEST_WINDOWS_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_REPCNT_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_REPCNTS_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_ADAPTP_HI_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_ADAPTP_LO_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_BUCKET_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_MARKOV_HI_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_MARKOV_LO_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_FW_OV_CONTROL_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_OBSERVE_FIFO_THRESH_OFFSET};
                                        }
                                    cr_cross: cross cp_lock_state, cp_offset;
                                endgroup: sw_update_cg
                                    covergroup sw_disable_cg with function sample(bit me_regwen, bit module_enable, entropy_src_main_sm_pkg::state_e main_sm_state);
                                    ...
                                    endgroup: sw_disable_cg
                                        covergroup enable_delay_cg with function sample(bit enable_i, bit esrng_fifo_not_empty_i, bit esbit_fifo_not_empty_i, bit postht_fifo_not_empty_i, bit distr_fifo_not_empty_i, bit sha3_block_busy_i, bit sha3_block_processed_i, bit bypass_mode_i, bit enable_o);
                                            ...
                                            cr_enable_i_fifo_state: cross cp_enable, cp_esrng_fifo, cp_esbit_fifo, cp_postht_fifo, cp_distr_fifo {
                                                    ...
                                                    illegal_bins enabling_and_any_fifo_not_empty = binsof (cp_enable.enabling)&&(binsof (cp_esrng_fifo.not_empty)||binsof (cp_esbit_fifo.not_empty)||binsof (cp_postht_fifo.not_empty)||binsof (cp_distr_fifo.not_empty));
                                                }
                                            cp_sha3_state: coverpoint {
                                                    sha3_block_busy_i, sha3_block_processed_i
                                                }
                                            {bins idle = {2'b00};
                                            ...
                                            cr_enable_i_sha3_state: cross cp_enable, cp_sha3_state {
                                                    ...
                                                    // impossible to cover these cases.  These cases are thus put into `illegal_bins`, so that
                                                    // they fail if the implementation changes and the coverage has to be redefined.
                                                    illegal_bins enabling_and_sha3_block_processed = binsof (cp_enable.enabling)&&binsof (cp_sha3_state.sha3_block_processed);
                                                }
                                        ...
                                        endgroup: enable_delay_cg
                                            ...
                                            covergroup win_ht_cg with function sample(health_test_e test_type, which_ht_e hi_lo, int window_size, bit fail);
                                                ...
                                                cr_cross: cross cp_winsize, cp_type, cp_hi_lo, cp_fail {
                                                        ...
                                                        ignore_bins ignore = binsof (cp_type) intersect {bucket_ht}&&binsof (cp_hi_lo) intersect {low_test};
                                                    }
                                            endgroup: win_ht_cg
                                                ...
                                                //          below.
                                                covergroup cont_ht_cg with function sample(health_test_e test_type, bit fips_mode, bit rng_bit_enable, bit [1:0] rng_bit_sel, bit [15:0] score, bit fail);
                                                ...
                                                endgroup: cont_ht_cg
                                                    ...
                                                    function automatic unsigned sigma_to_int(
                                                        real sigma
                                                    );
                                                        return unsigned'($rtoi($floor(sigma/sigma_res)));
                                                    endfunction
                                                    covergroup win_ht_deep_threshold_cg() with function sample(health_test_e test_type, which_ht_e hi_lo, int window_size, bit by_line, real sigma, bit fail);
                                                        ...
                                                        cr_cross: cross cp_winsize, cp_by_line, cp_type, cp_hi_lo, cp_fail, cp_threshold {
                                                                // bucket_ht does not have a low threshold
                                                                ignore_bins ignore_a = binsof (cp_type) intersect {bucket_ht}&&binsof (cp_hi_lo) intersect {low_test};
                                                                // by_line mode does not apply to bucket_ht
                                                                ignore_bins ignore_b = binsof (cp_type) intersect {bucket_ht}&&binsof (cp_by_line) intersect {1};
                                                            }
                                                    endgroup: win_ht_deep_threshold_cg
                                                        covergroup alert_cnt_cg with function sample(bit [15:0] threshold, bit has_fired);
                                                            ...
                                                            cr_cross: cross cp_threshold, cp_has_fired {
                                                                    ...
                                                                    ignore_bins ignore_a = binsof (cp_threshold) intersect {16'hffff}&&binsof (cp_has_fired) intersect {1};
                                                                }
                                                        endgroup: alert_cnt_cg
                                                            ...
                                                            endgroup: observe_fifo_threshold_cg
                                                                covergroup one_way_ht_threshold_reg_cg with function sample(int offset, bit rejected, bit is_fips);
                                                                    option.name = "one_way_ht_threshold_cg";
                                                                    option.per_instance = 1;
                                                                    cp_offset: coverpoint offset {
                                                                            bins one_way_regs[] = {entropy_src_reg_pkg::ENTROPY_SRC_REPCNT_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_REPCNTS_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_ADAPTP_HI_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_ADAPTP_LO_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_BUCKET_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_MARKOV_HI_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_MARKOV_LO_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_EXTHT_HI_THRESHOLD_OFFSET, entropy_src_reg_pkg::ENTROPY_SRC_EXTHT_LO_THRESHOLD_OFFSET};
                                                                        }
                                                                    ...
                                                                    endgroup: ht_watermark_num_cg
                                                                        ...
                                                                        endgroup: ht_watermark_cg
                                                                            ...
                                                                            endgroup: recov_alert_cg
                                                                                ...
                                                                                function automatic void cg_err_test_sample(
                                                                                    bit [4:0] err_code
                                                                                );
                                                                                    err_test_cg_inst.sample(err_code);
                                                                                endfunction

                                                                                function automatic void cg_mubi_err_sample(
                                                                                    invalid_mubi_e which_mubi
                                                                                );
                                                                                    mubi_err_cg_inst.sample(which_mubi);
                                                                                endfunction

                                                                                function automatic void cg_sm_err_sample(
                                                                                    bit ack_sm_err,
                                                                                    bit main_sm_err
                                                                                );
                                                                                ...
                                                                                function automatic void cg_fifo_err_sample(
                                                                                ...
                                                                                function automatic void cg_cntr_err_sample(
                                                                                    cntr_e which_cntr,
                                                                                    int which_line,
                                                                                    int which_bucket
                                                                                );
                                                                                    cntr_err_cg_inst.sample(
                                                                                                         which_cntr,
                                                                                                         which_line,
                                                                                                         which_bucket
                                                                                                     );
                                                                                ...
                                                                                function automatic void cg_seed_output_csr_sample(
                                                                                    mubi4_t fips_enable,
                                                                                    mubi4_t fips_flag,
                                                                                    mubi4_t rng_fips,
                                                                                    ...
                                                                                    seed_output_csr_cg_inst.sample(
                                                                                                                fips_enable,
                                                                                                                fips_flag,
                                                                                                                rng_fips,
                                                                                                                threshold_scope,
                                                                                ...
                                                                                function automatic void cg_csrng_hw_sample(
                                                                                    bit [3:0] fips_enable,
                                                                                    bit [3:0] fips_flag,
                                                                                    bit [3:0] rng_fips,
                                                                                ...
                                                                                function automatic void cg_observe_fifo_event_sample(
                                                                                    mubi4_t fips_enable,
                                                                                    mubi4_t fips_flag,
                                                                                    mubi4_t rng_fips,
                                                                                    mubi4_t threshold_scope,
                                                                                    ...
                                                                                    observe_fifo_event_cg_inst.sample(
                                                                                                                   fips_enable,
                                                                                                                   fips_flag,
                                                                                                                   rng_fips,
                                                                                                                   threshold_scope,
                                                                                ...
                                                                                function automatic void cg_sw_update_sample(
                                                                                    uvm_pkg::uvm_reg_addr_t offset,
                                                                                    bit sw_regupd,
                                                                                    bit module_enable
                                                                                );
                                                                                    string msg, fmt;

                                                                                    sw_update_cg_inst.sample(
                                                                                                          offset,
                                                                                                          sw_regupd,
                                                                                                          module_enable
                                                                                                      );
                                                                                ...
                                                                                function automatic void cg_sw_disable_sample(
                                                                                    bit me_regwen,
                                                                                    bit module_enable,
                                                                                    entropy_src_main_sm_pkg::state_e main_sm_state
                                                                                );
                                                                                    sw_disable_cg_inst.sample(
                                                                                                           me_regwen,
                                                                                                           module_enable,
                                                                                                           main_sm_state
                                                                                                       );
                                                                                ...
                                                                                function automatic void cg_cont_ht_sample(
                                                                                    health_test_e test_type,
                                                                                    bit fips_mode,
                                                                                    bit rng_bit_enable,
                                                                                    ...
                                                                                    symbol_score = (32'(raw_score)*`RNG_BUS_WIDTH>32'hffff)?32'hffff: 32'(raw_score)*`RNG_BUS_WIDTH;
                                                                                    score      = (test_type==repcnts_ht)?symbol_score[15:0]: raw_score;

                                                                                    cont_ht_cg_inst.sample(
                                                                                                        test_type,
                                                                                                        fips_mode,
                                                                                                        rng_bit_enable,
                                                                                                        rng_bit_select,
                                                                                ...
                                                                                function automatic void cg_win_ht_sample(
                                                                                    health_test_e test_type,
                                                                                    which_ht_e hi_low,
                                                                                    int window_size,
                                                                                    bit fail
                                                                                );
                                                                                    win_ht_cg_inst.sample(
                                                                                                       test_type,
                                                                                                       hi_low,
                                                                                                       window_size,
                                                                                                       fail
                                                                                ...
                                                                                function automatic void cg_win_ht_deep_threshold_sample(
                                                                                    health_test_e test_type,
                                                                                    which_ht_e hi_low,
                                                                                    int window_size,
                                                                                    bit by_line,
                                                                                    ...
                                                                                    win_ht_deep_threshold_cg_inst.sample(
                                                                                                                      test_type,
                                                                                                                      hi_low,
                                                                                                                      window_size,
                                                                                                                      by_line,
                                                                                ...
                                                                                function automatic void cg_alert_cnt_sample(
                                                                                    int threshold,
                                                                                    bit has_fired
                                                                                );
                                                                                ...
                                                                                function automatic void cg_observe_fifo_threshold_sample(
                                                                                    int threshold
                                                                                );
                                                                                    observe_fifo_threshold_cg_inst.sample(threshold);
                                                                                endfunction

                                                                                function automatic void cg_one_way_ht_threshold_reg_sample(
                                                                                    int offset,
                                                                                    bit rejected,
                                                                                    bit fips
                                                                                );
                                                                                    one_way_ht_threshold_reg_cg_inst.sample(
                                                                                                                         offset,
                                                                                                                         rejected,
                                                                                                                         fips
                                                                                                                     );
                                                                                ...
                                                                                function automatic void cg_recov_alert_sample(
                                                                                    int which_bit
                                                                                );
                                                                                    recov_alert_cg_inst.sample(which_bit);
                                                                                endfunction

                                                                                function automatic void cg_ht_watermark_num_sample(
                                                                                    logic [3:0] num
                                                                                );
                                                                                    ht_watermark_num_cg_inst.sample(num);
                                                                                endfunction

                                                                                function automatic void cg_ht_watermark_sample(
                                                                                    ht_watermark_num_e num,
                                                                                    logic [15:0] value
                                                                                );
                                                                                ...
                                                                                mubi4_t             fips_enable_csr, threshold_scope_csr, rng_bit_enable_csr, rng_bit_sel_csr, es_route_csr, es_type_csr, entropy_data_reg_enable_csr, fw_ov_mode_csr, entropy_insert_csr                  ;
                                                                                ...
                                                                                always @ ( posedge clk_i, negedge rst_ni ) begin
                                                                                    if (!rst_ni) begin
                                                                                        ...
                                                                                        if (csrng_if_req&&csrng_if_ack) begin
                                                                                            ...
                                                                                            cg_csrng_hw_sample(
                                                                                                entropy_src.reg2hw.conf.fips_enable.q,
