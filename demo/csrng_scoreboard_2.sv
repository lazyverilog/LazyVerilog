...
class csrng_scoreboard extends cip_base_scoreboard #(
    ...
    csrng_item          cs_item             []              ;
    push_pull_item #(.HostDataWidth(FIPS_CSRNG_BUS_WIDTH)) es_item[], es_item_q[][$];
    uint                                    more_cmd_data                         ;
    bit                 [TL_DW-1:0]         hw_genbits_reg_q      [$]             ;
    bit                 [GENBITS_BUS_WIDTH-1:0] hw_genbits, prd_genbits_q       [][$]          ;
    bit                 [CSRNG_BUS_WIDTH-1:0] cs_data               [], es_data             []              ;
    bit                                     fips                  []              ;
    ...
    function void build_phase(
        ...
        if (!uvm_config_db #(virtual csrng_cov_if)::get(
                                                        null,
                                                        "*.env",
                                                        "csrng_cov_if",
                                                        cov_vif
    ...
    virtual protected task handle_disable();
        forever begin
            wait(cfg.under_reset==0) csr_spinwait(.ptr(ral.ctrl.enable), .exp_data(MuBi4False), .backdoor(1), .timeout_ns(1_000_000_000) /* practically forever */);
    ...
    task automatic wait_es_item_or_disable(
        input uint app,
        output bit disabled
    );
        disabled   = 0;
        `DV_SPINWAIT(
            fork
            wait(es_item_q[app].size()>0); begin
                csr_spinwait(.ptr(ral.ctrl.enable), .exp_data(MuBi4False), .backdoor(1), .timeout_ns(1_000_000_000) /* practically forever */
                ); disabled = 1;
            end
        join_any
        disable fork;,
        ...
        case (csr.get_name())
            ...
            "cmd_req" : begin
                if (addr_phase_write) begin
                    ...
                    cov_vif.cg_cmds_sample(
                                SW_APP,
                                cs_item[SW_APP],
                                cmd_flag0_previous[SW_APP]
                            );
                    if (!more_cmd_data) begin
                        for (int i = 0 ; i<cs_item[SW_APP].cmd_data_q.size() ; i++) begin
                            cs_data[SW_APP] = (cs_item[SW_APP].cmd_data_q[i]<<i*CmdBusWidth)+cs_data[SW_APP];
                        end
                        case (cs_item[SW_APP].acmd)
                            INS: begin
                                ...
                                if (cs_item[SW_APP].flags!=MuBi4True) begin
                                    ...
                                    if (disabled) begin
                                        `uvm_info(`gfn, "Stopping to wait for entropy due to disable - Instantiate of SW_APP", UVM_MEDIUM)
                                        return;
                                    end
                            ...
                            RES: begin
                                ...
                                if (cs_item[SW_APP].flags!=MuBi4True) begin
                                    ...
                                    wait_es_item_or_disable(SW_APP, disabled);
                                    if (disabled) begin
                                        `uvm_info(`gfn, "Stopping to wait for entropy due to disable - Reseed of SW_APP", UVM_MEDIUM)
                                        return;
                                    end
                                    es_item[SW_APP] = es_item_q[SW_APP].pop_front;
                                    es_data[SW_APP] = es_item[SW_APP].d_data[CSRNG_BUS_WIDTH-1:0];
            ...
            "int_state_val" : begin
                ...
                if (data_phase_read) begin
                    ...
                    csr_rd(
                        .ptr(ral.int_state_num),
                        .value(int_state_num),
                        .backdoor(1'b1)
                    );
                    csr_rd(
                        .ptr(ral.int_state_read_enable),
                        .value(int_state_read_enable),
                        .backdoor(1'b1)
                    );
                    ...
                    if (`gmv(ral.ctrl.read_int_state)!=MuBi4True||cfg.otp_en_cs_sw_app_read!=MuBi8True||int_state_num>SW_APP||int_state_read_enable[int_state_num]==1'b0) begin
    ...
    function void ctr_drbg_update(
        uint app,
        bit [CSRNG_BUS_WIDTH-1:0] provided_data
    );
    ...
    function void ctr_drbg_instantiate(
        uint app,
        ...
        cfg.compliance[app] = fips||((`gmv(ral.ctrl.fips_force_enable)==MuBi4True)&&fips_force[app]);
        cov_vif.cg_csrng_state_db_sample(
                    cfg.compliance[app],
                    compliance_previous,
                    app
                );
    ...
    function void ctr_drbg_uninstantiate(
        uint app
    );
    ...
    function void ctr_drbg_generate(
        uint app,
        bit [11:0] glen,
        ...
        for (int i = 0 ; i<glen ; i++) begin
            ...
            if ((app!=SW_APP)||((cfg.sw_app_enable==MuBi4True)&&(cfg.otp_en_cs_sw_app_read==MuBi8True))) begin
    ...
    task collect_seeds();
        ...
        `DV_CHECK_FATAL(uvm_hdl_check_path(cmd_arb_idx_q_path)) forever begin
            ...
            endcase
            cov_vif.cg_csrng_es_sample(
                        es_item.d_data[CSRNG_BUS_WIDTH],
    ...
    task process_csrng_cmd_fifo(
        bit [MaxNumHwApps:0] app
    );
        forever begin
            ...
            for (int i = 0 ; i<cs_item[app].cmd_data_q.size() ; i++) begin
                cs_data[app] = (cs_item[app].cmd_data_q[i]<<i*CmdBusWidth)+cs_data[app];
            end
            cov_vif.cg_cmds_sample(
                        app,
                        cs_item[app],
                        cmd_flag0_previous[app]
                    );
            ...
            case (cs_item[app].acmd)
                INS: begin
                    ...
                    if (cs_item[app].flags!=MuBi4True) begin
                        ...
                        if (disabled) begin
                            `uvm_info(`gfn, $sformatf("Stopping to wait for entropy due to disable - Instantiate of app %0d", app), UVM_MEDIUM)
                            continue;
                        end
                ...
                RES: begin
                    ...
                    if (cs_item[app].flags!=MuBi4True) begin
                        ...
                        wait_es_item_or_disable(app, disabled);
                        if (disabled) begin
                            `uvm_info(`gfn, $sformatf("Stopping to wait for entropy due to disable - Reseed of app %0d", app), UVM_MEDIUM)
                            continue;
                        end
                        es_item[app] = es_item_q[app].pop_front();
                        es_data[app] = es_item[app].d_data[CSRNG_BUS_WIDTH-1:0];
