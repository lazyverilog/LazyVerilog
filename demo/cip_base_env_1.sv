...
function void cip_base_env::build_phase(
    uvm_phase phase
);
    ...
    if (cfg.is_chip==0) begin
        foreach (cfg.ral_models[i]) begin
            if (cfg.ral_models[i].has_shadowed_regs()&& !uvm_config_db #(rst_shadowed_vif)::get(
                                                                           this,
                                                                           "",
                                                                           "rst_shadowed_vif",
                                                                           cfg.rst_shadowed_vif
                                                                       )) begin
    ...
    foreach (cfg.m_tl_agent_cfgs[i]) begin
        ...
        m_tl_reg_adapters[i] = tl_reg_adapter #(tl_seq_item)::type_id::create({"m_tl_reg_adapter_", i});
        m_tl_reg_adapters[i].cfg = cfg.m_tl_agent_cfgs[i];
        uvm_config_db #(tl_agent_cfg)::set(
                                           this,
                                           $sformatf("m_tl_agent_%s*", i),
                                           "cfg",
    ...
    foreach (cfg.list_of_alerts[i]) begin
        ...
        uvm_config_db #(alert_esc_agent_cfg)::set(
                                                  this,
                                                  agent_name,
                                                  "cfg",
    ...
    if (cfg.num_edn) begin
        ...
        foreach (m_edn_pull_agent[i]) begin
            ...
            uvm_config_db #(edn_push_pull_agent_cfg_t)::set(
                                                            this,
                                                            agent_name,
                                                            "cfg",
                                                            cfg.m_edn_pull_agent_cfgs[i]
                                                        );
...
function void cip_base_env::connect_phase(
    uvm_phase phase
);
    ...
    foreach (cfg.list_of_alerts[i]) begin
        string alert_name = cfg.list_of_alerts[i];
        m_alert_agent[alert_name].monitor.alert_esc_port.connect(scoreboard.alert_fifos[alert_name].analysis_export);
    end
    ...
    foreach (cfg.list_of_alerts[i]) begin
        if (cfg.m_alert_agent_cfgs[cfg.list_of_alerts[i]].is_active) begin
            virtual_sequencer.alert_esc_sequencer_h[cfg.list_of_alerts[i]] = m_alert_agent[cfg.list_of_alerts[i]].sequencer;
        end
    end
...
function void cip_base_env::end_of_elaboration_phase(
    uvm_phase phase
);
