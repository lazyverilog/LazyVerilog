...
virtual task run_shadow_reg_errors(int num_times, bit en_csr_rw_seq=0);
    ...
    if (cfg.shadow_update_err_status_fields.size()==0||cfg.shadow_storage_err_status_fields.size()==0) begin
        `uvm_fatal(`gfn, "Please assign shadow reg status register fields in env_cfg!")
    end

    foreach (cfg.ral_models[i])
        cfg.ral_models[i].get_shadowed_regs(shadowed_csrs);
    ...
    if (en_csr_rw_seq) begin
        ...
        if (cfg.list_of_alerts.size()>0) begin
            ...
            csr_excl.add_excl(alert_test_reg.get_full_name(),
                              CsrExclWrite,
                              CsrRwTest);
        end
        ...
        foreach (shadowed_csrs[i]) begin
            ...
            csr_excl.add_excl(shadowed_csrs[i].get_full_name(),
                              CsrExclAll,
                              CsrRwTest);
        end
        ...
        foreach (cfg.shadow_update_err_status_fields[status_field]) begin
            ...
            csr_excl.add_excl(status_reg.get_full_name(),
                              CsrExclAll,
                              CsrRwTest);
        end
    ...
    for (int trans=1 ; trans<=num_times ; trans++) begin
        ...
        `uvm_info(`gfn, $sformatf("Running shadow reg error test iteration %0d/%0d. max_idx=%0d", trans, num_times, max_idx), UVM_LOW)

        shadowed_csrs.shuffle();

        for (int i=0 ; i<=max_idx ; ++i) begin
            if (cfg.stop_transaction_generators())
                break;
            `uvm_info(`gfn, $sformatf("mycsr: %s    en_csr_rw_seq:%d", shadowed_csrs[i].get_name(), en_csr_rw_seq), UVM_HIGH)
            ...
            repeat (5) begin
                ...
                fork
                    begin
                        ...
                        1: begin
                            shadow_reg_err_ctrl_svas(.enable(0));
                            glitch_shadowed_reset(shadowed_csrs,
                                                  ready_to_trigger_csr_rw,
                                                  has_fatal_alert);
                        end
                    endcase
                end
...
virtual task write_and_check_update_error(dv_base_reg shadowed_csr);
    ...
    shadow_reg_wr(.csr(shadowed_csr),
                  .wdata(wdata),
                  .en_shadow_wr(0));
    ...
    `uvm_info(`gfn, $sformatf("write %0s with first value %0h second value %0h", shadowed_csr.get_name(), wdata, err_wdata), UVM_HIGH)
...
virtual task check_csr_read_clear_staged_val(dv_base_reg shadowed_csr);
    ...
    if (cfg.m_alert_agent_cfgs.exists(alert_name)) begin
        `DV_CHECK_EQ(cfg.m_alert_agent_cfgs[alert_name].vif.get_alert(), 0, $sformatf("Unexpected alert: %s fired", alert_name))
    end
...
virtual task poke_and_check_storage_error(dv_base_reg shadowed_csr);
    ...
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(kind, kind inside {BkdrRegPathRtl, BkdrRegPathRtlShadow};)
    origin_val = csr_peek(.ptr(shadowed_csr), .kind(kind));
    err_val    = get_shadow_reg_diff_val(shadowed_csr, origin_val);

    csr_poke(.ptr(shadowed_csr),
             .value(err_val),
             .kind(kind),
             .predict(1));
    predict_shadow_reg_status(.predict_storage_err(shadowed_csr.get_shadow_storage_err()));
    `uvm_info(`gfn, $sformatf("backdoor write %s through %s with value 0x%0h", shadowed_csr.`gfn, kind.name, err_val), UVM_HIGH)
...
virtual task glitch_shadowed_reset(ref dv_base_reg shadowed_csr[$], ref bit ready_to_trigger_csr_rw, ref bit has_fatal_alert);
    string alert_name;
    ...
    foreach (shadowed_csr[i]) begin
        if (shadowed_csr[i].get_reset()!=`gmv(shadowed_csr[i])) begin
            ...
            if (alert_name=="")
                alert_name = "fatal_err";
            `uvm_info(`gfn, $sformatf("Expect reset storage error %0s", shadowed_csr[i].get_name()), UVM_HIGH)
            break;
        end
    end
...
virtual function bit [BUS_DW-1:0] get_shadow_reg_diff_val(dv_base_reg csr, bit [BUS_DW-1:0] origin_val);
    ...
    if ($urandom_range(0, 1)) begin
        ...
        `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(get_shadow_reg_diff_val, (get_shadow_reg_diff_val&reg_mask)!=(origin_val&reg_mask);)
    end
    else begin
...
// finished.
virtual task shadow_reg_wr(dv_base_reg csr, uvm_reg_data_t wdata, bit en_shadow_wr=1, bit predict=1);
    ...
    fork
        begin
            repeat (en_shadow_wr?2: 1)
                csr_wr_for_shadow_reg_predict(csr,
                                              wdata,
                                              predict);
...
virtual function void predict_shadow_reg_status(bit predict_update_err=0, bit predict_storage_err=0);
    ...
    foreach (cfg.shadow_update_err_status_fields[status_field]) begin
        ...
        if (status_field.get_access()=="RC") begin
            if (!status_field.predict(.value(0), .kind(UVM_PREDICT_DIRECT)))
                `uvm_error(`gfn, $sformatf("Failed to predict clear of %0s after read.", status_field.get_full_name()))
        end
    end
    foreach (cfg.shadow_storage_err_status_fields[status_field]) begin
        csr_rd_check(.ptr(status_field),
