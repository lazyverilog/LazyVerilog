...
class i2c_scoreboard extends cip_base_scoreboard #(
        ...
        case (cfg.m_i2c_agent_cfg.if_mode)
            ...
            Device: begin
                ...
                forever begin
                    `DV_SPINWAIT_EXIT(
                    // WAIT_
                    wait(cfg.clk_rst_vif.rst_n===1'b1) fork
    ...
    virtual task process_tl_access(tl_seq_item item, tl_channels_e channel, string ral_name);
        ...
        if (cfg.en_cov) begin
            if (tl_putdata)
                sample_write_coverage(item.a_data, csr);
            if (tl_accessackdata)
                sample_read_coverage(item.d_data, csr);
        end
    ...
    virtual task sample_read_coverage(bit [bus_params_pkg::BUS_DW-1:0] data, uvm_reg csr);
        ...
        case (csr.get_name())
            ...
            "status" : begin
                cov.status_cg.sample(.fmtfull(`gmv(ral.status.fmtfull)),
            ...
            "ovrd" : begin
                cov.scl_sda_override_cg.sample(.txovrden(`gmv(ral.ovrd.txovrden)),
    ...
    task sample_fmt_fifo_data(
        ref i2c_item dut_trn
    );
        i2c_item            fmt_data                            ;
        ...
        cov.fmt_fifo_cg.sample(.fbyte(fmt_data.fbyte),
        ...
        `uvm_info(`gfn, $sformatf("fmt_data.size = %0d ; dut_trn.size = %0d", fmt_fifo_data_q.size(), dut_trn.data_q.size()), UVM_MEDIUM)

        if (dut_trn.bus_op!=BusOpRead) begin
            foreach (dut_trn.data_q[i]) begin
                fmt_data = fmt_fifo_data_q.pop_front();
                cov.fmt_fifo_cg.sample(.start(fmt_data.start),
                                       .stop(fmt_data.stop),
                                       .read(fmt_data.read),
                                       .rcont(fmt_data.rcont),
                                       .nakok(fmt_data.nakok),
    ...
    task collect_scl_stretch_cg();
        fork
            forever begin
                ...
                join_any
                csr_rd(.ptr(ral.target_fifo_status.acqlvl),
                       .value(acqlvl),
                       .backdoor(UVM_BACKDOOR));
                csr_rd(.ptr(ral.target_fifo_status.txlvl),
                       .value(txlvl),
                       .backdoor(UVM_BACKDOOR));
                cov.scl_stretch_cg.sample(.host_mode(`gmv(ral.ctrl.enablehost)),
    ...
    task compare_target_write_trans();
        ...
        if (cfg.en_cov) begin cov.sample_i2c_b2b_cg(obs_wr.addr,
                                                    `gmv(ral.ctrl.enablehost));
        end
    ...
    task compare_target_read_trans();
        ...
        // Sample the covergroup before doing the comparison.
        if (cfg.en_cov) begin cov.sample_i2c_b2b_cg(obs_rd.addr,
                                                    `gmv(ral.ctrl.enablehost));
        end
    ...
    //
    task compare_controller_trans(
        bus_op_e dir=BusOpWrite
    );
        i2c_item            exp                                 , obs                                 ;
        ...
        if (cfg.en_cov) begin
            cov.sample_i2c_b2b_cg(obs.addr,
                                  `gmv(ral.ctrl.enablehost));
        ...
        if (!obs.compare(exp)) begin
            `uvm_error(`gfn, $sformatf("Miscompare: DUT-Controller, dir:%0s\n--> EXP:\n%0s\--> OBS:\n%0s", dir.name(), exp.sprint(), obs.sprint()))
        end
        else begin
            `uvm_info(`gfn, $sformatf("Compare succeeded: DUT-Controller, dir:%0s\n--> EXP:\n%0s\--> OBS:\n%0s", dir.name(), exp.sprint(), obs.sprint()), UVM_DEBUG)
        end
