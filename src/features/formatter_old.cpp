...
// Field-ownership migration model.  Legacy passes still use Tok::fmt_* as a
// scratch view while they run, but run_field_owned_pass() captures their diff
// into exactly one domain below and then recomposes the render cache from an
// immutable snapshot of already-committed domains.  That makes pass writes
// exclusive by metadata field/domain instead of by token region.
enum class LayoutFieldDomain {
    Passthrough,
    Structure,
    HeaderLists,
    Comments,
...
struct FormatMetadataSnapshot {
    ...
    bool fmt_newline_after{false};
    int fmt_blank_lines{0};
    bool fmt_passthrough{false};
};

struct FormatMetadataPatch {
    std::optional<int> fmt_indent;
    std::optional<int> fmt_spaces_before;
    std::optional<std::string> fmt_text_before;
    std::optional<bool> fmt_newline_before;
    std::optional<bool> fmt_newline_after;
    std::optional<int> fmt_blank_lines;
    std::optional<bool> fmt_passthrough;

    bool empty() const {
        return !fmt_indent && !fmt_spaces_before && !fmt_text_before &&
               !fmt_newline_before && !fmt_newline_after && !fmt_blank_lines &&
               !fmt_passthrough;
    }
};
...
struct Tok {
    ...
    LayoutRegion layout_region{LayoutRegion::None};

    // --- Render cache (scratch while a legacy pass runs; composed afterward) ---
    ...
    bool fmt_newline_after{false};  // emit newline immediately after this token's text
    ...
    // Exclusive-writer field domains.  A pass may write only its assigned
    // FormatMetadataPatch.  render_tokens() reads only the composed fmt_* cache.
    std::vector<FormatMetadataPatch> layout_fields;
...
static void assign_layout_regions_pass(std::vector<Tok>& tokens);
...
static void format_comment_layout_pass(std::vector<Tok>& tokens, const FormatOptions& opts) {
    // Pass 1: set baseline fmt_* for every Comment-region token using its pre-classified
    // comment_role.  The port-list fixup below may override specific cases.
    for (size_t i = 0; i < tokens.size(); ++i) {
        if (!tok_comment(tokens[i])) continue;
        if (tokens[i].layout_region != LayoutRegion::Comment) continue;
        auto& ctok = tokens[i];

        // Scan backward through whitespace to detect a newline before this comment.
        ...
        // Multi-line block comments: if preceded by a line comment, force a new
        // line so the block opener is not swallowed by the line comment.
        if (tok_block_comment(ctok) && tok_text(ctok).find('\n') != std::string::npos) {
            ...
            // Fall through to role-based handling below (no fmt_passthrough).
        }

        // Helper: indent from next Basic token.
        auto indent_from_next_basic = [&]() -> int {
            for (size_t j = i + 1; j < tokens.size(); ++j) {
                if (tok_whitespace(tokens[j])) continue;
                if (tokens[j].layout_region == LayoutRegion::Basic)
                    return tokens[j].fmt_indent;
            }
            return 0;
        };

        // Helper: count blank lines from source whitespace before this comment.
        auto blank_lines_from_source = [&]() -> int {
            bool has_prev_non_ws = false;
            for (size_t j = i; j > 0; --j) {
                if (!tok_whitespace(tokens[j - 1])) { has_prev_non_ws = true; break; }
            }
            if (!has_prev_non_ws) return 0;
            int blank = 0;
            for (size_t j = i; j > 0; --j) {
                if (!tok_whitespace(tokens[j - 1])) break;
            ...
            return std::min(blank, opts.blank_lines_between_items);
        };

        switch (ctok.comment_role) {
        case CommentRole::LeadingStatement:
            ctok.fmt_newline_before = true;
            ctok.fmt_newline_after  = false;
            ctok.fmt_spaces_before  = 0;
            ctok.fmt_indent         = indent_from_next_basic();
            ctok.fmt_blank_lines    = blank_lines_from_source();
            break;
        case CommentRole::OwnLineInterstitial:
            ctok.fmt_newline_before = true;
            ctok.fmt_newline_after  = true;
            ctok.fmt_spaces_before  = 0;
            ctok.fmt_indent         = indent_from_next_basic();
            ctok.fmt_blank_lines    = blank_lines_from_source();
            break;
        case CommentRole::TrailingStatement:
            ...
            ctok.fmt_indent         = 0;
            break;
        case CommentRole::TrailingInterstitial:
            ctok.fmt_newline_before = false;
            ctok.fmt_newline_after  = true;
            ctok.fmt_spaces_before  = 1;
            ctok.fmt_blank_lines    = 0;
            ctok.fmt_indent         = 0;
            break;
        case CommentRole::LeadingInterstitial:
            ctok.fmt_newline_before = source_newline_before_comment;
            ctok.fmt_newline_after  = false;
            ctok.fmt_spaces_before  = source_newline_before_comment ? 0 : 1;
            ctok.fmt_blank_lines    = 0;
            ctok.fmt_indent         = source_newline_before_comment ? indent_from_next_basic() : 0;
            break;
        case CommentRole::InlineInterstitial:
            ctok.fmt_newline_before = false;
            ctok.fmt_newline_after  = false;
            ctok.fmt_spaces_before  = 1;
            ctok.fmt_blank_lines    = 0;
            ctok.fmt_indent         = 0;
            break;
        case CommentRole::None:
        default:
            // Fallback for line comments with no stable role (e.g. in disabled regions).
            if (tok_line_comment(ctok)) {
                // Use source position to decide standalone vs. inline.
                // (These are typically in passthrough/disabled regions.)
                ctok.fmt_newline_before = source_newline_before_comment;
                ctok.fmt_spaces_before  = source_newline_before_comment ? 0 : 1;
                ctok.fmt_blank_lines    = 0;
                ctok.fmt_indent         = source_newline_before_comment ? indent_from_next_basic() : 0;
...
static void format_portlist_pass(std::vector<Tok>& tokens, const FormatOptions& opts) {
    for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        if (token_range_has_pp_conditional(tokens, i, semi + 1) ||
            token_range_has_line_directive(tokens, port_open, port_close + 1) ||
        ...
        if (hash_idx != SIZE_MAX && param_close != SIZE_MAX &&
            !token_range_has_line_directive(tokens, hash_idx, param_close + 1)) {
        ...
        for (size_t j = port_open + 1; j < port_close; ++j) {
            ...
            if (tok_is(tokens[j], ",", TokenKind::Comma) && paren == 0 && bracket == 0 && brace == 0) {
                ...
                if (comma_after_own_line_comment) {
                    ...
                    // not a trailing delimiter classified for the comment line.  Keep
        ...
        for (size_t ei = 0; ei < entries.size(); ++ei) {
            ...
            // When putting a newline before this non-ANSI entry, any inline
            // block comment between the previous comma and this entry should
            // start the new line rather than trail the previous one.
            // E.g. "a, b, /* cfg */ cdefghij" should wrap as:
            //   a, b,
            //   /* cfg */ cdefghij
        ...
        // Format comments classified for ModuleHeaderComment inside the param/port ranges.
        // These are emitted by the header-list field domain, so comment layout has one writer.
        auto format_header_list_comments = [&](size_t open_idx, size_t close_idx) {
            for (size_t j = open_idx + 1; j < close_idx; ++j) {
                if (!tok_comment(tokens[j]) || tokens[j].layout_region != LayoutRegion::ModuleHeaderComment)
                    continue;
                // Detect whether a newline precedes this comment in the source.
                bool src_nl = false;
                for (size_t k = j; k > 0; --k) {
                    if (!tok_whitespace(tokens[k - 1])) break;
                    if (tok_text(tokens[k - 1]).find('\n') != std::string::npos) { src_nl = true; break; }
                }
                if (tokens[j].comment_role == CommentRole::LeadingInterstitial &&
                    tok_block_comment(tokens[j])) {
                    size_t prev_sig = prev_code_sig(tokens, open_idx + 1, j);
                    bool after_sep = prev_sig != SIZE_MAX &&
                                     tok_is(tokens[prev_sig], ",", TokenKind::Comma);
                    size_t owner_idx = tokens[j].comment_owner;
                    if (after_sep && owner_idx != SIZE_MAX && owner_idx < tokens.size() &&
                        tokens[owner_idx].fmt_newline_before) {
                        tokens[j].fmt_newline_before = true;
                        tokens[j].fmt_blank_lines = 0;
                        tokens[j].fmt_indent = base_indent + 1;
                        tokens[j].fmt_spaces_before = 0;
                        tokens[j].fmt_newline_after = false;
                        tokens[owner_idx].fmt_newline_before = false;
                        tokens[owner_idx].fmt_spaces_before = 1;
                    } else if (src_nl) {
                        tokens[j].fmt_newline_before = true;
                        tokens[j].fmt_blank_lines = 0;
                        tokens[j].fmt_indent = base_indent + 1;
                    ...
                    bool own_line = comment_role_own_line_rendered(tokens[j].comment_role) || src_nl;
                    if (own_line) {
                        tokens[j].fmt_newline_before = true;
                        tokens[j].fmt_blank_lines = 0;
                        tokens[j].fmt_indent = base_indent + 1;
        ...
        if (hash_idx != SIZE_MAX && param_close != SIZE_MAX &&
            tokens[param_open].layout_region == LayoutRegion::ModuleHeaderParameterList)
            format_header_list_comments(param_open, param_close);
        if (tokens[port_open].layout_region == LayoutRegion::ModuleHeaderPortList)
    ...
    auto finalize_line = [&](size_t end_idx) {
        ...
        for (size_t k = cur_line_start; k < end_idx; ++k) {
            ...
            if (tokens[k].in_disabled_region || tokens[k].fmt_passthrough ||
                ...
                tokens[k].layout_region == LayoutRegion::ModuleHeaderParameterList ||
                tokens[k].layout_region == LayoutRegion::ModuleHeaderPortList ||
                tokens[k].layout_region == LayoutRegion::ModuleHeaderComment)
                ln.disabled = true;
    ...
    while (li < lines.size()) {
        ...
        for (auto& e : block) {
            ...
            for (size_t k = 0; k < nd; ++k) {
                ...
                // Set spacing between name and trail (section3 → section4)
                if (!decl.trail_str.empty() &&
                    decl.trail_start != decl.trail_end && decl.trail_start < tokens.size()) {
                    int name_pad = std::max(1, id_w - (int)decl.name_str.size());
                    tokens[decl.trail_start].fmt_spaces_before = name_pad;
                }

                if (!is_last) {
                    // Comma token is at decl.trail_end; pad it to end of section3+section4
                    if (decl.trail_end < tokens.size()) {
                        int pad;
                        if (decl.trail_str.empty())
                            pad = std::max(1, id_w - (int)decl.name_str.size() + trail_w);
                        else
                            pad = std::max(1, trail_w - (int)decl.trail_str.size());
                        tokens[decl.trail_end].fmt_spaces_before = pad;
                    }
                    // One space after comma before next declarator name
                    if (k + 1 < nd)
                        tokens[vp.declarators[k + 1].name_idx].fmt_spaces_before = 1;
                } else {
...
static std::string render_tokens(const std::vector<Tok>& tokens, const FormatOptions& opts) {
    ...
    for (const auto& tok : tokens) {
        ...
        if (tok.fmt_newline_after && !at_bol) {
            out += '\n';
            at_bol = true;
            line_comment_open = false;
        }
...
static bool format_metadata_changed(const Tok& tok, const FormatMetadataSnapshot& before) {
    return tok.fmt_indent != before.fmt_indent ||
           ...
           tok.fmt_newline_after != before.fmt_newline_after ||
           tok.fmt_blank_lines != before.fmt_blank_lines ||
           tok.fmt_passthrough != before.fmt_passthrough;
}

static void restore_format_metadata(Tok& tok, const FormatMetadataSnapshot& snap) {
    tok.fmt_indent = snap.fmt_indent;
    tok.fmt_spaces_before = snap.fmt_spaces_before;
    tok.fmt_text_before = snap.fmt_text_before;
    tok.fmt_newline_before = snap.fmt_newline_before;
    tok.fmt_newline_after = snap.fmt_newline_after;
    tok.fmt_blank_lines = snap.fmt_blank_lines;
    tok.fmt_passthrough = snap.fmt_passthrough;
}

static void ensure_layout_field_domains(Tok& tok) {
    size_t n = static_cast<size_t>(LayoutFieldDomain::Count);
    if (tok.layout_fields.size() != n)
        tok.layout_fields.resize(n);
}

static void apply_patch(FormatMetadataSnapshot& snap, const FormatMetadataPatch& patch) {
    if (patch.fmt_indent) snap.fmt_indent = *patch.fmt_indent;
    if (patch.fmt_spaces_before) snap.fmt_spaces_before = *patch.fmt_spaces_before;
    if (patch.fmt_text_before) snap.fmt_text_before = *patch.fmt_text_before;
    if (patch.fmt_newline_before) snap.fmt_newline_before = *patch.fmt_newline_before;
    if (patch.fmt_newline_after) snap.fmt_newline_after = *patch.fmt_newline_after;
    if (patch.fmt_blank_lines) snap.fmt_blank_lines = *patch.fmt_blank_lines;
    if (patch.fmt_passthrough) snap.fmt_passthrough = *patch.fmt_passthrough;
}

static void compose_layout_field_cache(std::vector<Tok>& tokens) {
    for (auto& tok : tokens) {
        ensure_layout_field_domains(tok);
        FormatMetadataSnapshot composed;
        for (size_t d = 0; d < static_cast<size_t>(LayoutFieldDomain::Count); ++d)
            apply_patch(composed, tok.layout_fields[d]);
        restore_format_metadata(tok, composed);
    }
}

static FormatMetadataPatch diff_format_metadata(const FormatMetadataSnapshot& before,
                                                const FormatMetadataSnapshot& after) {
    FormatMetadataPatch patch;
    if (after.fmt_indent != before.fmt_indent) patch.fmt_indent = after.fmt_indent;
    if (after.fmt_spaces_before != before.fmt_spaces_before) patch.fmt_spaces_before = after.fmt_spaces_before;
    if (after.fmt_text_before != before.fmt_text_before) patch.fmt_text_before = after.fmt_text_before;
    if (after.fmt_newline_before != before.fmt_newline_before) patch.fmt_newline_before = after.fmt_newline_before;
    if (after.fmt_newline_after != before.fmt_newline_after) patch.fmt_newline_after = after.fmt_newline_after;
    if (after.fmt_blank_lines != before.fmt_blank_lines) patch.fmt_blank_lines = after.fmt_blank_lines;
    if (after.fmt_passthrough != before.fmt_passthrough) patch.fmt_passthrough = after.fmt_passthrough;
    return patch;
}

static const char* layout_field_domain_name(LayoutFieldDomain domain) {
    switch (domain) {
        case LayoutFieldDomain::Passthrough: return "passthrough";
        case LayoutFieldDomain::Structure: return "structure";
        case LayoutFieldDomain::HeaderLists: return "header_lists";
        case LayoutFieldDomain::Comments: return "comments";
        case LayoutFieldDomain::ClassExtends: return "class_extends";
        case LayoutFieldDomain::AssignmentAlignment: return "assignment_alignment";
        case LayoutFieldDomain::VariableAlignment: return "variable_alignment";
        case LayoutFieldDomain::PortAlignment: return "port_alignment";
        case LayoutFieldDomain::EnumLayout: return "enum_layout";
        case LayoutFieldDomain::ModportLayout: return "modport_layout";
        case LayoutFieldDomain::InstanceLayout: return "instance_layout";
        case LayoutFieldDomain::CallLayout: return "call_layout";
        case LayoutFieldDomain::CovergroupLayout: return "covergroup_layout";
        case LayoutFieldDomain::ConstraintLayout: return "constraint_layout";
        case LayoutFieldDomain::FunctionDeclLayout: return "function_decl_layout";
        case LayoutFieldDomain::Count: break;
    }
    return "unknown";
}

struct LayoutFieldPipeline {
    std::vector<bool> domain_written =
        std::vector<bool>(static_cast<size_t>(LayoutFieldDomain::Count), false);
};

template <typename Fn>
static void run_field_owned_pass(const char* pass_name, LayoutFieldDomain domain,
                                 LayoutFieldPipeline& pipeline, std::vector<Tok>& tokens,
                                 Fn&& fn) {
    const size_t d = static_cast<size_t>(domain);
    if (d >= static_cast<size_t>(LayoutFieldDomain::Count))
        throw std::logic_error(std::string(pass_name) + " uses an invalid layout field domain");
    if (pipeline.domain_written[d]) {
        std::ostringstream oss;
        oss << pass_name << " attempted a second write to layout field domain '"
            << layout_field_domain_name(domain) << "'";
        throw std::logic_error(oss.str());
    }
    pipeline.domain_written[d] = true;

    // Snapshot read model: every pass reads the render cache composed from the
    // previously committed domains. Its in-place fmt_* writes are scratch only.
    compose_layout_field_cache(tokens);
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        ensure_layout_field_domains(tokens[i]);
        FormatMetadataSnapshot after = snapshot_format_metadata(tokens[i]);
        FormatMetadataPatch patch = diff_format_metadata(before[i], after);
        if (!patch.empty())
            tokens[i].layout_fields[d] = std::move(patch);
        restore_format_metadata(tokens[i], before[i]);
    }
    compose_layout_field_cache(tokens);
}
...
static void basic_formatting(std::vector<Tok>& tokens, const std::string& input,
    ...
    auto classify_paren_at = [&](size_t open_idx, bool procedural_at_context) -> ParenKind {
        ...
        // Check if this is a module/interface/program declaration port list.
        // Pattern: module [import ...] [#(...)] (ports) ;
        // Only at top paren level; and only when preceded by ) from #(...) or an
        // identifier — not by keywords like wait/if/while which are never module names.
        // Guard: left must be ) (param list close) to avoid false-positives on
        // wait(done), if(...), etc. inside module bodies.
        if (paren_stack.empty() &&
            tok_is(left, ")", TokenKind::CloseParenthesis)) {
            size_t close_idx = matching_close_paren(tokens, open_idx, tokens.size());
            if (close_idx != SIZE_MAX) {
                size_t semi = next_code_sig(tokens, close_idx + 1, tokens.size());
                if (semi != SIZE_MAX && tok_is(tokens[semi], ";", TokenKind::Semicolon)) {
                    bool found_module = false;
                    size_t scan_limit = open_idx > 1000 ? open_idx - 1000 : 0;
                    size_t k = open_idx;
                    while (k > scan_limit) {
                        --k;
                        const auto& tk = tokens[k];
                        if (tok_whitespace(tk) || tok_comment(tk) || tok_directive(tk)) continue;
                        // Jump over matching parens (e.g. #(...) parameter list)
                        if (tok_is(tk, ")", TokenKind::CloseParenthesis)) {
                            size_t po = matching_open_paren(k);
                            if (po == SIZE_MAX) break;
                            k = po;
                            continue;
                        }
                        TokenKind knd = tok_kind(tk);
                        if (knd == TokenKind::ModuleKeyword ||
                            knd == TokenKind::MacromoduleKeyword ||
                            knd == TokenKind::InterfaceKeyword ||
                            knd == TokenKind::ProgramKeyword) {
                            found_module = true;
                            break;
                        }
                        // Stop at tokens that cannot appear in a module header prefix
                        if (knd == TokenKind::EndModuleKeyword ||
                            knd == TokenKind::EndInterfaceKeyword ||
                            knd == TokenKind::EndProgramKeyword ||
                            knd == TokenKind::BeginKeyword ||
                            knd == TokenKind::EndKeyword ||
                            knd == TokenKind::AlwaysKeyword ||
                            knd == TokenKind::AlwaysCombKeyword ||
                            knd == TokenKind::AlwaysFFKeyword ||
                            knd == TokenKind::AlwaysLatchKeyword ||
                            knd == TokenKind::InitialKeyword ||
                            knd == TokenKind::TaskKeyword ||
                            knd == TokenKind::FunctionKeyword ||
                            knd == TokenKind::ClassKeyword ||
                            tok_is(tk, "{", TokenKind::OpenBrace) ||
                            tok_is(tk, "}", TokenKind::CloseBrace) ||
                            tok_is(tk, "=", TokenKind::Equals) ||
                            tok_is(tk, "<=", TokenKind::LessThanEquals))
                            break;
                    }
                    if (found_module)
                        return ParenKind::ModuleHeaderPortList;
                }
            }
        }
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        if (tok_whitespace(tok)) {
            ...
            if (nl > 0 && paren_depth > 0) {
                bool in_module_port_list = paren_depth == 1 && !paren_stack.empty() &&
                                           paren_stack.back() == ParenKind::ModuleHeaderPortList;
                if (!in_module_port_list)
                    continue;
                // Inside a module header port list: treat newlines as significant
                // so each port appears on its own line.
                pending_nl = true;
            }
        ...
        // --- D. Comment tokens are entirely classified for format_comment_layout_pass.
        // Let pending_nl and blank_pend flow to the next Basic token.
        if (tok_comment(tok)) {
            prev = &tok;
            original_newline_before_token = false;
            continue;
        ...
        const bool is_basic_region = (tok.layout_region == LayoutRegion::Basic);
        if (dec == SD::MustWrap) {
            pending_nl = false;
            if (is_basic_region) tok.fmt_newline_before = true;
            if (is_basic_region) tok.fmt_blank_lines = blank_pend;
            blank_pend = 0;
            at_bol = true;
            ...
            if (!at_bol && spaces > 0 && is_basic_region)
                tok.fmt_spaces_before = spaces;
        } else {
            apply_newline(i, is_basic_region);
            if (!at_bol && spaces > 0 && is_basic_region)
                tok.fmt_spaces_before = spaces;
        }
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        if (tok_whitespace(tokens[i]))
            continue;
        if (tokens[i].layout_region == LayoutRegion::Basic) {
    ...
    // local to Basic-region tokens so field-domain assertions still catch any
    // accidental cross-pass mutation.
    size_t prev_sig_idx = SIZE_MAX;
    for (size_t i = 0; i < tokens.size(); ++i) {
        if (tok_whitespace(tokens[i]) || tok_comment(tokens[i]) || tok_directive(tokens[i]))
            continue;
        if (prev_sig_idx != SIZE_MAX && tokens[i].layout_region == LayoutRegion::Basic) {
...
static void passthrough_regions_pass(std::vector<Tok>& tokens, const FormatOptions& opts) {
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        if (!in_passthrough) {
            ...
            // When the nearest non-whitespace predecessor is a define-block token
            // its text already ends with '\n', so every whitespace newline between
            // the two blocks is a blank line (no -1 adjustment needed).
            int blank = 0;
            int last_ws_nl = 0;
            bool prev_nonws_is_define = false;
            for (size_t j = i; j > 0; --j) {
                if (!tok_whitespace(tokens[j - 1])) {
                    prev_nonws_is_define = tok_define_block(tokens[j - 1]);
                    break;
                }
                last_ws_nl = (int)std::count(tok_text(tokens[j - 1]).begin(),
                                             tok_text(tokens[j - 1]).end(), '\n');
                blank = std::max(0, last_ws_nl - 1);
            }
            if (prev_nonws_is_define)
                blank = last_ws_nl;
...
static void set_layout_region_if_unowned(std::vector<Tok>& tokens, size_t first, size_t last,
                                         LayoutRegion owner) {
    for (size_t k = first; k < last && k < tokens.size(); ++k) {
        if (tok_whitespace(tokens[k]))
            continue;
        // Directive tokens (e.g. `ifdef, `endif) are always classified for Basic so
        // basic_formatting can emit them as passthrough regardless of context.
        if (tok_directive(tokens[k]))
            continue;
        if (tokens[k].layout_region == LayoutRegion::None)
            tokens[k].layout_region = owner;
    }
}

static void assign_layout_regions_pass(std::vector<Tok>& tokens) {
    // Strong token regions first.  Comments are intentionally classified for the
    ...
    for (auto& tok : tokens) {
        if (tok.in_disabled_region) {
            tok.layout_region = LayoutRegion::DisabledRegion;
        } else if (tok_define_block(tok)) {
            tok.layout_region = LayoutRegion::DefineBlock;
        } else if (tok_whitespace(tok)) {
            continue;
        } else if (tok_comment(tok)) {
            tok.layout_region = LayoutRegion::Comment;
        }
    }
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
        ...
        // Classify ParameterList/PortList regions per-subrange so that a line
        // directive in the parameter list (e.g. `include) does not prevent the
        // port list from being formatted by format_portlist_pass.
        bool param_range_ok = hash_idx != SIZE_MAX && param_close != SIZE_MAX &&
            !token_range_has_pp_conditional(tokens, hash_idx, param_close + 1) &&
            !token_range_has_line_directive(tokens, hash_idx, param_close + 1) &&
            !token_range_disabled_or_passthrough(tokens, hash_idx, param_close + 1);
        bool port_range_ok =
            !token_range_has_pp_conditional(tokens, port_open, port_close + 1) &&
            !token_range_has_line_directive(tokens, port_open, port_close + 1) &&
            !token_range_disabled_or_passthrough(tokens, port_open, port_close + 1);
        if (param_range_ok) {
            set_layout_region_if_unowned(tokens, hash_idx, param_close + 1,
                                        LayoutRegion::ModuleHeaderParameterList);
            for (size_t k = hash_idx; k < param_close + 1; ++k)
                if (tok_comment(tokens[k]) && !tokens[k].in_disabled_region)
                    tokens[k].layout_region = LayoutRegion::ModuleHeaderComment;
        }
        if (port_range_ok) {
            set_layout_region_if_unowned(tokens, port_open, port_close + 1,
                                        LayoutRegion::ModuleHeaderPortList);
            for (size_t k = port_open; k < port_close + 1; ++k)
                if (tok_comment(tokens[k]) && !tokens[k].in_disabled_region)
                    tokens[k].layout_region = LayoutRegion::ModuleHeaderComment;
        }
        set_layout_region_if_unowned(tokens, i, semi + 1, LayoutRegion::Basic);
        i = semi;
    }

    for (auto& tok : tokens) {
        if (!tok_whitespace(tok) && tok.layout_region == LayoutRegion::None)
            tok.layout_region = LayoutRegion::Basic;
    }
}

static const char* layout_region_name(LayoutRegion owner) {
    switch (owner) {
        case LayoutRegion::None: return "None";
        case LayoutRegion::Basic: return "Basic";
        case LayoutRegion::Comment: return "Comment";
        case LayoutRegion::DisabledRegion: return "DisabledRegion";
        case LayoutRegion::DefineBlock: return "DefineBlock";
        case LayoutRegion::ModuleHeaderParameterList: return "ModuleHeaderParameterList";
        case LayoutRegion::ModuleHeaderPortList: return "ModuleHeaderPortList";
        case LayoutRegion::ModuleHeaderComment: return "ModuleHeaderComment";
    }
    return "Unknown";
}
...
std::string format_source(const std::string& source, const FormatOptions& opts) {
    ...
    assign_layout_regions_pass(tokens);
    normalization_pass(tokens);

    // Field dependency DAG for the migrated formatter:
    //   immutable lexemes / syntax facts / source comment roles
    //        ↓
    //   passthrough regions
    //        ↓
    //   structure baseline (wrapping + indentation + default spacing)
    //        ↓
    //   list/comment reflow domains
    //        ↓
    //   alignment domains
    //        ↓
    //   syntax-specific refinements
    //        ↓
    //   render
    // There are no reverse edges from rendered columns back into wrapping.
    LayoutFieldPipeline field_pipeline;
    compose_layout_field_cache(tokens);
    write_log(opts, "01_normalization_pass.sv", render_tokens(tokens, opts));

    run_field_owned_pass("passthrough_regions_pass", LayoutFieldDomain::Passthrough,
                         field_pipeline, tokens,
                         [&] { passthrough_regions_pass(tokens, opts); });
    run_field_owned_pass("basic_formatting", LayoutFieldDomain::Structure,
                         field_pipeline, tokens,
                         [&] { basic_formatting(tokens, input, opts); });
    write_log(opts, "02_basic_formatting.sv", render_tokens(tokens, opts));

    run_field_owned_pass("format_portlist_pass", LayoutFieldDomain::HeaderLists,
                         field_pipeline, tokens,
                         [&] { format_portlist_pass(tokens, opts); });
    write_log(opts, "03_format_portlist_pass.sv", render_tokens(tokens, opts));
    run_field_owned_pass("format_comment_layout_pass", LayoutFieldDomain::Comments,
                         field_pipeline, tokens,
                         [&] { format_comment_layout_pass(tokens, opts); });
    write_log(opts, "04_format_comment_layout_pass.sv", render_tokens(tokens, opts));
    run_field_owned_pass("format_class_extends_parameter_pass", LayoutFieldDomain::ClassExtends,
                         field_pipeline, tokens,
                         [&] { format_class_extends_parameter_pass(tokens, opts); });
    write_log(opts, "05_format_class_extends_parameter_pass.sv", render_tokens(tokens, opts));

    if (opts.statement.align) {
        run_field_owned_pass("align_assign_pass", LayoutFieldDomain::AssignmentAlignment,
                             field_pipeline, tokens,
                             [&] { align_assign_pass(tokens, opts); });
        write_log(opts, "06_align_assign_pass.sv", render_tokens(tokens, opts));
    }
    if (opts.var_declaration.align) {
        run_field_owned_pass("align_var_pass", LayoutFieldDomain::VariableAlignment,
                             field_pipeline, tokens,
                             [&] { align_var_pass(tokens, opts); });
        write_log(opts, "07_align_var_pass.sv", render_tokens(tokens, opts));
    }
    if (opts.port_declaration.align) {
        run_field_owned_pass("align_port_pass", LayoutFieldDomain::PortAlignment,
                             field_pipeline, tokens,
