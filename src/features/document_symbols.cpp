...
struct DocumentShape {
    ...
    // Declarations generated by a macro *defined in this same file*, keyed by
    // name and holding the invocation site (1-based line, 0-based column).
    //
    // The index records where a declaration was written, which for a macro body
    // is the `define` itself.  That is the useful answer for a shared header's
    // macro -- it is genuinely where the declaration lives -- but for a local
    // macro it puts every generated symbol on the `define` line, so the outline
    // cannot tell `lane_0_valid` from `lane_1_valid` and neither one reveals
    // anything when clicked.  A cross-file macro already reports the invocation,
    // so this also makes the two cases agree.
    std::unordered_map<std::string, std::pair<int, int>> macro_decl_sites;
...
struct ShapeVisitor : slang::syntax::SyntaxVisitor<ShapeVisitor> {
    ...
    Extent extent_of(const slang::syntax::SyntaxNode& node) const {
        ...
        return Extent{(int)line, lsp_column(sm, end)};
    }
    ...
    /// Record the invocation site of a declarator that came from a macro body.
    void note_macro_declarator(const slang::parsing::Token& name) {
        if (!name || !name.location().valid())
            return;
        if (!sm.isMacroLoc(name.location()) || sm.isMacroArgLoc(name.location()))
            return;
        ...
        if (current && !block_stack.empty() &&
            ...
            for (const auto* decl : node.declarators) {
                ...
                block.decls.push_back(BlockDecl{std::string(decl->name.valueText()), type,
                                                ...
                                                lsp_column(sm, decl->name.location())});
            }
        }
        visitDefault(node);
    }
...
lsDocumentSymbol make_symbol(const std::string& name, lsSymbolKind kind, int line, int col,
    ...
    sym.range.end = lsPosition(l, col + lsp_column_width(name));
    ...
    for (const auto& mod : index.modules) {
        ...
        for (const auto& v : index.values) {
            if (v.parent_scope != mod.name || !in_document(v.file_id))
                continue;
            // A declaration generated by a macro defined in this file is recorded
            // at the `define`; report it where it was invoked instead.
            int v_line = v.line;
            int v_col = v.col;
            if (const auto site = shape.macro_decl_sites.find(v.name);
                site != shape.macro_decl_sites.end()) {
                v_line = site->second.first;
                v_col = site->second.second;
            }
            if (v.kind == "variable") {
                push_child(target(v_line), make_symbol(v.name, lsSymbolKind::Variable, v_line,
                                                       v_col, opt_str(v.type)));
